...
import com.my_medi.api.user.dto.UserResponseDto.RequestingUserInfoDto;
...
//    }
    ...
    public ApiResponseDto<RequestingUserInfoDto> getUserInfo(@AuthExpert Expert expert,
                                                             @PathVariable Long userId,
                                                             @RequestParam(name = "status") RequestStatus status) // REQUESTED/ACCEPTED/REJECTED
    {
        ...
        return ApiResponseDto.onSuccess(UserConverter.toRequestingUserInfoDto(user, proposal, latestReport));
    }
